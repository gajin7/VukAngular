...
export class ProfileComponent implements OnInit {
  ...
  mockProfile() {
    this.profileForm.setValue({
      firstName: "Jovan",
      lastName: "Gajin",
      email: "gajinjovan@gmail.com",
      type: this.getTypeById(3),
      lastAppointment: this.datePipe.transform(new Date(), "dd:MM:yyyy"),
  ...
  getUserInfo() {
    this.authWebService.getUserInfo().subscribe((data) => {
      ...
      this.profileForm.setValue({
        ...
        email: profile.Email,
        type: profile.Type,
        lastAppointment: this.datePipe.transform(
          profile.LastAppointment,
          "dd:MM:yyyy"
        ),
        nextAppointment: this.datePipe.transform(
          profile.SuggestedAppointment,
          "dd:MM:yyyy"
        ),
      });
    });
